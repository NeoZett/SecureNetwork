...
class SERVER(Server): 
    ...
    def __init__(self, key: bytes | None, hmac_key: bytes | None, address: _Address, on_event: Callable[[Event], Any], max_clients: Optional[int] = None):
        ...
        self.mfa_enabled_users = {}  # {username: {"secret": TOTP_SECRET, "method": "email" or "sms", "contact": "email_or_phone"}}
    ...
    def create_account(self, username: str, password: str, role: str = "user", mfa_method: str = None, contact: str = None) -> bool:
        ...
        if mfa_method in ["email", "sms"]:
            ...
            print(f"🔐 MFA enabled for {username} via {mfa_method}")
    ...
    def _is_valid_session(self, username: str, token: str) -> bool:
        ...
        if received_hmac != calculated_hmac:
            print(f"Token verification failed for {username}")
    ...
    def _send_sms(self, phone_number: str, code: str):
        ...
        try:
            ...
            print(f"📲 MFA code sent to {phone_number}: {response.json()}")
    ...
    def _send_email(self, recipient: str, code: str):
        ...
        smtp_port = 587
        sender_email = "20091103neo@gmail.com"
        sender_password = "NeoZ5517-09"
        ...
        try:
            ...
            server.quit()
            print(f"📧 MFA code sent to {recipient}")
        except Exception as e:
    ...
    def _intercept_event(self, event: Event) -> None:
        ...
        if event.type == EventType.RECIEVED:
            try:
                ...
                if not addr:
                    print("⚠️ No address found for sendto(). Dropping message.")
                    return  # Prevent crash

                # Print debug info
                print(f"Debug: Received data for user {username}")
                if token:
                    print(f"Debug: Token provided: {token}")
                    if username in self.accounts:
                        print(f"Debug: Stored token: {self.accounts[username].get('token')}")
                ...
                if data.get("mfa_code"):
                    if username in self.mfa_enabled_users:
                        ...
                        if totp.verify(data["mfa_code"]):
                            print(f"✅ MFA success for {username}")
                        else:
                            print(f"❌ Incorrect MFA code for {username}")
                ...
                if username and password and not token:
                    ...
                    if len(self.failed_logins[ip_address]) >= self.RATE_LIMIT:
                        print(f"🚫 Too many failed attempts from {ip_address}. Blocking for {self.BLOCK_TIME} seconds.")
                    ...
                    if username in self.accounts and bcrypt.checkpw(password.encode(), self.accounts[username]["password"].encode()):
                        ...
                        print(f"Debug: Generated new token: {session_token}")  # Debug line
                ...
                if "message" in data:
                    if self._is_valid_session(username, token):
                        ...
                        new_event = Event(EventType.RECIEVED, 
                            {"data": data["message"].encode(), "username": username, "token": token}
                        )
                        self.on_event_callback(new_event)
                        return
                    else:
                        print(f"❌ Unauthorized message from {username}")
                        self.on_event_callback(Event(EventType.ERROR, {"exception": "Invalid or expired session"}))
                        return

            except Exception as e:
                print(f"⚠️ Error processing message: {e.__class__.__name__}: {str(e)} (steams from line {e.__traceback__.tb_lineno})")
                return
        ...
        if event.type == EventType.RECIEVED:
            try:
                ...
                if data.get("status") == "success":
                    ...
                    print(f"✅ Authenticated! Token received: {self.token}")
                else:
                    print(f"❌ Authentication failed: {data.get('message', 'Unknown error')}")
            except json.JSONDecodeError as e:
                print("Data was not server response, parsing through.")
            except Exception as e:
                print(f"⚠️ Error processing server response: {e.__class__.__name__}: {str(e)} (steams from line {e.__traceback__.tb_lineno})")
                return
        self.on_event_callback(event)
